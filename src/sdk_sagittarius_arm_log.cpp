...
void log_print(int level, char const *log_format, ...)
{
    ...
    {
    case LOG_TYPE_DEBUG:
        printf("\e[0;32m""DEBUG: ""\e[0;37m""%s""\e[0m", log_print_buff);
        break;
    case LOG_TYPE_INFO:
        printf("\e[0;37m""INFO: ""%s""\e[0m", log_print_buff);
        break;
    case LOG_TYPE_WARN:
        printf("\e[0;33m""WARN: ""%s""\e[0m", log_print_buff);
        break;
    case LOG_TYPE_ERROR:
        printf("\e[1;31m""ERROR: ""%s""\e[0m", log_print_buff);
        break;
...
///                             1: ERROR
///                             0: None
void log_set_level(int level) { if (level >= 0 && level <= 4) log_level = level; }
