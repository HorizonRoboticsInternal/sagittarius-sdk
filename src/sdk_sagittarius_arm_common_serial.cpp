...
{
    CSDarmCommonSerial::CSDarmCommonSerial(const std::string &serialname, int &baudrate, int &timelimit) : CSDarmCommon(),
    ...
    int CSDarmCommonSerial::SendArmAllServerTime(float v1, float v2, float v3, float v4, float v5, float v6)
    {
        unsigned char buf[30];
        ...
        if (mFd == -1)
        {
            log_print(LOG_TYPE_ERROR, "SendArmAllServerTime: Serial NOT open\n");
            return -1;
        }
        buf[0] = 0x55;                                 //帧头1 55
        buf[1] = 0xAA;                                 //帧头2 AA
        ...
        log_print(LOG_TYPE_INFO, "time:%dms [%.1f,%.1f,%.1f,%.1f,%.1f,%.1f]\n", 0, (float(short(buf[7] | (buf[8] << 8)))) / 10, (float(short(buf[9] | (buf[10] << 8)))) / 10, (float(short(buf[11] | (buf[12] << 8)))) / 10, (float(short(buf[13] | (buf[14] << 8)))) / 10, (float(short(buf[15] | (buf[16] << 8)))) / 10, (float(short(buf[17] | (buf[18] << 8)))) / 10);
    ...
    int CSDarmCommonSerial::SendArmAllServerWithIndex(ServoStruct sv[], int num)
    {
        ...
        if (mFd == -1)
        {
            log_print(LOG_TYPE_ERROR, "SendArmAllServerWithIndex: Serial NOT open\n");
            return -1;
        }
        buf[0] = 0x55;                  //帧头1 55
        buf[1] = 0xAA;                  //帧头2 AA
        ...
        //    ROS_INFO("[%d,%d,%d,%d,%d,%d]", short(buf[5]|(buf[6]<<8)),short(buf[7]|(buf[8]<<8)),short(buf[9]|(buf[10]<<8)),short(buf[11]|(buf[12]<<8)),short(buf[13]|(buf[14]<<8)),short(buf[15]|(buf[16]<<8)));
        //log_print(LOG_TYPE_INFO, "[%.1f,%.1f,%.1f,%.1f,%.1f,%.1f]", (float(short(buf[5]|(buf[6]<<8))))/10,(float(short(buf[7]|(buf[8]<<8))))/10,(float(short(buf[9]|(buf[10]<<8))))/10,(float(short(buf[11]|(buf[12]<<8))))/10,(float(short(buf[13]|(buf[14]<<8))))/10,(float(short(buf[15]|(buf[16]<<8))))/10);
    ...
    int CSDarmCommonSerial::SendArmAllServer(float v1, float v2, float v3, float v4, float v5, float v6)
    {
        ...
        buf[17] = CheckSum(buf);         //校验和
        buf[18] = 0x7D;                  //结束位
        //printf("[%.1f,%.1f,%.1f,%.1f,%.1f,%.1f]", (float(short(buf[5]|(buf[6]<<8))))/10,(float(short(buf[7]|(buf[8]<<8))))/10,(float(short(buf[9]|(buf[10]<<8))))/10,(float(short(buf[11]|(buf[12]<<8))))/10,(float(short(buf[13]|(buf[14]<<8))))/10,(float(short(buf[15]|(buf[16]<<8))))/10);
        if (memcmp(buf, lastbuf, 19) != 0)
        {
            memcpy(lastbuf, buf, 19);
            if (SendSerialData2Arm((char *)buf, 19) != 19)
    ...
    int CSDarmCommonSerial::GetDataGram(unsigned char *receiveBuffer, int bufferSize, int *length)
    {
        ...
        if (mFd == -1)
        {
            ...
            {
            	first_bit = 0;
            	log_print(LOG_TYPE_ERROR, "GetDataGram: Serial not open\n");  //只打印一次，使用红色打印。
            }
            return -1;
        }
        ...
        if (1) //(!stream_stopped_)
        {
            ...
            {
                log_print(LOG_TYPE_WARN, "GetDataGram: serial-receive timeout for %ds\n", mTimeLimit);  //warnning!
                return -1;
            }
        }
