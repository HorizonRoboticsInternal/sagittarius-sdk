...
namespace sdk_sagittarius_arm
{
    ...
    SagittariusArmReal::SagittariusArmReal(std::string strSerialName, int baudrate, int vel=500, int acc=5)
    {
        int result;
        int iTimeLimit = 1;
        pSDKarm = NULL;
        ...
        memcpy(lower_joint_limits, tmp_lower_joint_limits, JOINT_NUM * sizeof(float));
        memcpy(upper_joint_limits, tmp_upper_joint_limits, JOINT_NUM * sizeof(float));

        pSDKarm = new sdk_sagittarius_arm::CSDarmCommonSerial(strSerialName, baudrate, iTimeLimit);
        result = pSDKarm->Init();
    ...
    bool SagittariusArmReal::CheckUpperLower(float js[])
    {
        ...
        {
            if((js[i]<lower_joint_limits[i])||(js[i]>upper_joint_limits[i]))
            {
    ...
    bool SagittariusArmReal::CheckUpperLowerWithIndex(ServoStruct sv[], int num)
    {
        ...
        for(i=0; i<num; i++)
        {
            if((sv[i].value<lower_joint_limits[sv[i].id])||(sv[i].value>upper_joint_limits[sv[i].id]))
            {
                log_print(LOG_TYPE_ERROR, "joint[%d] is not in the range\n",sv[i].id);
                return false;
            }        
    ...
    {
        if(pSDKarm != NULL)
            pSDKarm->SetArmTorque(arm_torque);
        return true;
    }  
    ...
    {
        if(CheckUpperLower(joint_positions)==false)
            return;
        if(pSDKarm!=NULL)
        {
    ...
    {
        if(CheckUpperLowerWithIndex(sv, num)==false)
            return;
        if(pSDKarm!=NULL)
        {
            if(torque_status)
    ...
    SagittariusArmKinematics::SagittariusArmKinematics(float x, float y, float z)
    {
        // NO.     w(x,    y,      z)     q(x,        y,           z)
        ...
        Eigen::Vector3d w1(0, 0, -1), w2(0, -1, 0), w3(0, -1, 0), w4(-1, 0, 0), w5(0, -1, 0), w6(-1, 0, 0);                                                                              //角速度方向
        Eigen::Vector3d q1(0.045, 0, 0.07), q2(0.045, -0.0001, 0.125), q3(0.078, -0.0002, 0.304), q4(0.1625, -0.0002, 0.304), q5(0.24685, 0.00045, 0.304), q6(0.308237, 0.00045, 0.304); //螺旋轴位置
        Eigen::VectorXd s1(6, 1), s2(6, 1), s3(6, 1), s4(6, 1), s5(6, 1), s6(6, 1);                                                                                                      //螺旋轴序列
        s1 << w1, -w1.cross(q1);
        s2 << w2, -w2.cross(q2);
        s3 << w3, -w3.cross(q3);
        s4 << w4, -w4.cross(q4);
    ...
    /// @param eomg - 姿态的精度
    /// @param ev - 位置的精度
    bool SagittariusArmKinematics::getIKinThetaMatrix(const Eigen::MatrixXd& M_EE, float theta_result[], double eomg, double ev)
    {
        Eigen::VectorXd theta(JOINT_NUM);
        theta << 0, 0, 0, 0, 0, 0;
        // theta << theta_result[0], theta_result[1], theta_result[2], theta_result[3], theta_result[4], theta_result[5];
        ...
        log_print(LOG_TYPE_DEBUG, "joint theta: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f\n", theta_result[1], theta_result[2], theta_result[3], theta_result[4], theta_result[5], theta_result[6]);
    ...
    /// @param eomg - 姿态的精度
    /// @param ev - 位置的精度
    bool SagittariusArmKinematics::getIKinThetaEuler(float x, float y, float z, float roll, float pitch, float yaw, float theta_result[], double eomg, double ev)
    {
        // 欧拉角转旋转矩阵
        Eigen::AngleAxisd yawAxis = Eigen::AngleAxisd(yaw * M_PI / 180, Eigen::Vector3d::UnitZ());
        Eigen::AngleAxisd pitchAxis = Eigen::AngleAxisd(pitch * M_PI / 180, Eigen::Vector3d::UnitY());
        Eigen::AngleAxisd rollAxis = Eigen::AngleAxisd(roll * M_PI / 180, Eigen::Vector3d::UnitX());
        Eigen::Matrix3d R = yawAxis.matrix() * pitchAxis.matrix() * rollAxis.matrix();

        Eigen::Isometry3d T = Eigen::Isometry3d::Identity(); // 变换矩阵, 实质为4*4的矩阵
        T.rotate(R);                                         // 输入旋转矩阵
    ...
    /// @param ev - 位置的精度，单位是米
    bool SagittariusArmKinematics::getIKinThetaQuaternion(float x, float y, float z, float ox, float oy, float oz, float ow, float theta_result[], double eomg, double ev)
    {
        // 四元数转旋转矩阵
    ...
    bool SagittariusArmKinematics::getFKinMatrix(float theta[], Eigen::MatrixXd& M_EE)
    {
    ...
    bool SagittariusArmKinematics::getFKinEuler(float theta[], float xyz[], float euler[])
    {
    ...
    bool SagittariusArmKinematics::getFKinQuaternion(float theta[], float xyz[], float quaternion[])
    {
        Eigen::MatrixXd T = Eigen::MatrixXd::Identity(4, 4);
        Eigen::Matrix3d R;
        getFKinMatrix(theta, T);
