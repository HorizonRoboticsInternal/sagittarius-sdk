...
int main(int argc, char** argv)
{
    ...
    while(1)
    {
        ...
        Eigen::MatrixXd FK_T = Eigen::MatrixXd::Identity(4, 4);     // 变换矩阵的单位矩阵
        sgr_kinematics.getFKinMatrix(joint_positions, FK_T);        // 对 joint_positions 进行正向运动学
        log_print(LOG_TYPE_INFO, "Forward Kinematics matrix from joint value\n");
        std::cout << FK_T << std::endl;

        sar.GetCurrentJointStatus(js);                              // 获取当前角度
        sgr_kinematics.getFKinMatrix(js, FK_T);                     // 对 js 进行正向运动学，返回矩阵
        ...
        // 获取末端在目标点上时每个舵机的角度，使用XYZ位置与欧拉角姿态
        log_print(LOG_TYPE_INFO, "target XYZ and Euler:\n");
        log_print(LOG_TYPE_INFO, "x=%.4f y=%.4f z=%.4f roll=%.4f pitch=%.4f yaw=%.4f\n", 0.3, 0.0, 0.1, 0.0, 80.0, 0.0);
        if(sgr_kinematics.getIKinThetaEuler(0.3, 0, 0.1, 0, 60, 0, joint_positions)) 
        {
            sar.SetAllServoRadian(joint_positions);         //设置6个舵机的弧度
            sleep(5);
        ...
        log_print(LOG_TYPE_INFO, "x=%.4f y=%.4f z=%.4f ox=%.4f oy=%.4f oz=%.4f, ow=%.4f\n", 0.3, 0.0, 0.1, 0.0, 0.3826, 0.0, 0.9238);
        if(sgr_kinematics.getIKinThetaQuaternion(0.3, 0, 0.1, 0, 0.3826, 0, 0.9238, joint_positions)) 
        {
            sar.SetAllServoRadian(joint_positions);                 //设置6个舵机的弧度
            sleep(5);
            ...
            sar.GetCurrentJointStatus(js);                          // 获取当前角度
            sgr_kinematics.getFKinQuaternion(js, xyz, quaternion);  // 对 js 进行正向运动学，返回位置和四元素姿态
            log_print(LOG_TYPE_INFO, "x=%.4f y=%.4f z=%.4f ox=%.4f oy=%.4f oz=%.4f, ow=%.4f\n", xyz[0], xyz[1], xyz[2], quaternion[0], quaternion[1], quaternion[2], quaternion[3]);
            sleep(2);
        }
        ...
        Eigen::Matrix3d R = yawAxis.matrix() * pitchAxis.matrix() * rollAxis.matrix(); // 欧拉角转旋转矩阵
