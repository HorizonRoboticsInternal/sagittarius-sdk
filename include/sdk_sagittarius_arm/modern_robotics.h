...
namespace mr
{
     ...
     * velocity vector Input: Eigen::Vector3d 3x1 angular velocity vector
     * Returns: Eigen::MatrixXd 3x3 skew symmetric matrix
     */
    Eigen::Matrix3d VecToso3(const Eigen::Vector3d&);

    /*
     * Function: Returns angular velocity vector represented by the skew
     * symmetric matrix Inputs: Eigen::MatrixXd 3x3 skew symmetric matrix
     * Returns: Eigen::Vector3d 3x1 angular velocity
     */
    Eigen::Vector3d so3ToVec(const Eigen::MatrixXd&);

    /*
     * Function: Tranlates an exponential rotation into it's individual
     *components Inputs: Exponential rotation (rotation matrix in terms of a
     *rotation axis and the angle of rotation) Returns: The axis and angle of
     *rotation as [x, y, z, theta]
     */
     ...
     * 				Special Euclidian Group (SE3) homogeneous transformation
     *matrix Inputs: Rotation Matrix (R), Position Vector (p) Returns: Matrix of
     *T = [ [R, p], [0, 1] ]
     */
    ...
    /*
     * Function: Translates a spatial velocity vector into a transformation
     * matrix Inputs: Spatial velocity vector [angular velocity, linear
     * velocity] Returns: Transformation matrix
     */
    Eigen::MatrixXd VecTose3(const Eigen::VectorXd&);

    /* Function: Translates a transformation matrix into a spatial velocity
     * vector Inputs: Transformation matrix Returns: Spatial velocity vector
     ...
     *			 Used to change the frame of reference for spatial velocity
     *vectors Inputs: 4x4 Transformation matrix SE(3) Returns: 6x6 Adjoint
     ...
     * Inputs: se3 matrix representation of exponential coordinates
     * (transformation matrix) Returns: 6x6 Matrix representing the rotation
     */
    Eigen::MatrixXd MatrixExp6(const Eigen::MatrixXd&);

    /*
     * Function: Computes the matrix logarithm of a homogeneous transformation
     * matrix Inputs: R: Transformation matrix in SE3 Returns: The matrix
     * logarithm of R
     */
     ...
     *end-effector The joint screw axes in the space frame when the manipulator
     *             is at the home position
     * 		   A list of joint coordinates.
     * Returns: Transfomation matrix representing the end-effector frame when
     *the joints are at the specified coordinates Notes: FK means Forward
     *Kinematics
     */
     ...
     *calculation) Inputs: Home configuration (position and orientation) of
     *end-effector The joint screw axes in the body frame when the manipulator
     *             is at the home position
     * 		   A list of joint coordinates.
     * Returns: Transfomation matrix representing the end-effector frame when
     *the joints are at the specified coordinates Notes: FK means Forward
     *Kinematics
     */
    Eigen::MatrixXd FKinBody(const Eigen::MatrixXd&, const Eigen::MatrixXd&,
                             const Eigen::VectorXd&);
     ...
     */
    Eigen::MatrixXd JacobianSpace(const Eigen::MatrixXd&,
     ...
     */
    Eigen::MatrixXd JacobianBody(const Eigen::MatrixXd&,
                                 const Eigen::MatrixXd&);
    ...
    /*
     * Function: Returns the Frobenius norm to describe the distance of M from
     *the SO(3) manifold Inputs: M: A 3x3 matrix Outputs: the distance from mat
     *to the SO(3) manifold using the following method: If det(M) <= 0, return a
     *large number. If det(M) > 0, return norm(M^T*M - I).
     */
    double DistanceToSO3(const Eigen::Matrix3d&);

    /*
     * Function: Returns the Frobenius norm to describe the distance of mat from
     *the SE(3) manifold Inputs: T: A 4x4 matrix Outputs: the distance from T to
     *the SE(3) manifold using the following method: Compute the determinant of
     *matR, the top 3x3 submatrix of T. If det(matR) <= 0, return a large
     *number. If det(matR) > 0, replace the top 3x3 submatrix of mat with
     *matR^T*matR, and set the first three entries of the fourth column of mat
     *to zero. Then return norm(T - I).
     */
     ...
     *robot Inputs: Blist: The joint screw axes in the end-effector frame when
     *the manipulator is at the home position, in the format of a matrix with
     *axes as the columns M: The home configuration of the end-effector T: The
     *desired end-effector configuration Tsd thetalist[in][out]: An initial
     *guess and result output of joint angles that are close to satisfying Tsd
     ...
     *	thetalist[in][out]: Joint angles that achieve T within the specified
     *tolerances,
     */
    bool IKinBody(const Eigen::MatrixXd&, const Eigen::MatrixXd&,
                  const Eigen::MatrixXd&, Eigen::VectorXd&, double, double);

    /*
     * Function: Computes inverse kinematics in the space frame for an open
     *chain robot Inputs: Slist: The joint screw axes in the space frame when
     *the manipulator is at the home position, in the format of a matrix with
     *axes as the columns M: The home configuration of the end-effector T: The
     *desired end-effector configuration Tsd thetalist[in][out]: An initial
     *guess and result output of joint angles that are close to satisfying Tsd
     ...
     *tolerances,
     */
    bool IKinSpace(const Eigen::MatrixXd&, const Eigen::MatrixXd&,
                   const Eigen::MatrixXd&, Eigen::VectorXd&, double, double);

    /*
     * Function: This function uses forward-backward Newton-Euler iterations to
     * solve the equation: taulist = Mlist(thetalist) * ddthetalist +
     ...
     */
    Eigen::VectorXd
    InverseDynamics(const Eigen::VectorXd&, const Eigen::VectorXd&,
    ...
    /*
     * Function: This function calls InverseDynamics with Ftip = 0, dthetalist =
     * 0, and ddthetalist = 0. The purpose is to calculate one important term in
     * the dynamics equation Inputs: thetalist: n-vector of joint variables g:
     * Gravity vector g Mlist: List of link frames {i} relative to {i-1} at the
     * home position Glist: Spatial inertia matrices Gi of the links Slist:
     * Screw axes Si of the joints in a space frame, in the format of a matrix
    ...
    /*
     * Function: This function calls InverseDynamics n times, each time passing
     * a ddthetalist vector with a single element equal to one and all other
    ...
    /*
     * Function: This function calls InverseDynamics with g = 0, dthetalist = 0,
     * and ddthetalist = 0.
     *
     * Inputs:
     *  thetalist: n-vector of joint variables
    ...
    /*
     * Function: Compute the joint angles and velocities at the next timestep
     ...
     *
     * Outputs:
     *  thetalist[out]: Vector of joint variables after dt from first order
     Euler integration
     *  dthetalist[out]: Vector of joint rates after dt from first order Euler
     integration
     */
    void EulerStep(Eigen::VectorXd&, Eigen::VectorXd&, const Eigen::VectorXd&,
                   double);

    /*
     * Function: Compute the joint forces/torques required to move the serial
     *chain along the given trajectory using inverse dynamics Inputs: thetamat:
     *An N x n matrix of robot joint variables (N: no. of trajecoty time step
     *points; n: no. of robot joints dthetamat: An N x n matrix of robot joint
     *velocities ddthetamat: An N x n matrix of robot joint accelerations g:
     *Gravity vector g Ftipmat: An N x 6 matrix of spatial forces applied by the
     *end-effector (if there are no tip forces the user should input a zero
     *matrix) Mlist: List of link frames {i} relative to {i-1} at the home
     *position Glist: Spatial inertia matrices Gi of the links Slist: Screw axes
     *Si of the joints in a space frame, in the format of a matrix with the
     *screw axes as the columns.
     *
     * Outputs:
     *  taumat: The N x n matrix of joint forces/torques for the specified
     *trajectory, where each of the N rows is the vector of joint forces/torques
     *at each time step
     */
    Eigen::MatrixXd InverseDynamicsTrajectory(
    ...
    /*
     * Function: Compute the motion of a serial chain given an open-loop history
     *of joint forces/torques Inputs: thetalist: n-vector of initial joint
     *variables dthetalist: n-vector of initial joint rates taumat: An N x n
     *matrix of joint forces/torques, where each row is is the joint effort at
     *any time step g: Gravity vector g Ftipmat: An N x 6 matrix of spatial
     *forces applied by the end-effector (if there are no tip forces the user
     *should input a zero matrix) Mlist: List of link frames {i} relative to
     ...
     *(Euler) takes places between each time step. Must be an integer value
    ...
    ForwardDynamicsTrajectory(const Eigen::VectorXd&, const Eigen::VectorXd&,
     ...
     *
     * Outputs:
     *  tau_computed: The vector of joint forces/torques computed by the
     *feedback linearizing controller at the current instant
     */
    Eigen::VectorXd
    ComputedTorque(const Eigen::VectorXd&, const Eigen::VectorXd&,
                   const Eigen::VectorXd&, const Eigen::VectorXd&,
                   const std::vector<Eigen::MatrixXd>&,
                   const std::vector<Eigen::MatrixXd>&, const Eigen::MatrixXd&,
                   const Eigen::VectorXd&, const Eigen::VectorXd&,
                   const Eigen::VectorXd&, double, double, double);
     ...
     */
    Eigen::MatrixXd JointTrajectory(const Eigen::VectorXd&,
                                    const Eigen::VectorXd&, double, int, int);

    /*
     * Function: Compute a trajectory as a list of N SE(3) matrices
     *corresponding to the screw motion about a space screw axis Inputs: Xstart:
     *The initial end-effector configuration Xend: The final end-effector
     *configuration Tf: Total time of the motion in seconds from rest to rest N:
     *The number of points N > 1 (Start and stop) in the discrete representation
     *of the trajectory method: The time-scaling method, where 3 indicates cubic
     *(third- order polynomial) time scaling and 5 indicates quintic
     *          (fifth-order polynomial) time scaling
     *
     * Outputs:
     ...
     *        and the Nth is Xend
     */
    std::vector<Eigen::MatrixXd> ScrewTrajectory(const Eigen::MatrixXd&,
                                                 const Eigen::MatrixXd&, double,
                                                 int, int);

    /*
     * Function: Compute a trajectory as a list of N SE(3) matrices
     *corresponding to the origin of the end-effector frame following a straight
     *line Inputs: Xstart: The initial end-effector configuration Xend: The
     *final end-effector configuration Tf: Total time of the motion in seconds
     ...
     *  end-effector frame follows a straight line, decoupled from the
     *rotational motion.
     */
     ...
     * Function: Compute the motion of a serial chain given an open-loop history
     *of joint forces/torques Inputs: thetalist: n-vector of initial joint
     *variables dthetalist: n-vector of initial joint rates g: Gravity vector g
     *	Ftipmat: An N x 6 matrix of spatial forces applied by the end-effector
     *(if there are no tip forces the user should input a zero matrix) Mlist:
     *List of link frames {i} relative to {i-1} at the home position Glist:
     *Spatial inertia matrices Gi of the links Slist: Screw axes Si of the
     *joints in a space frame, in the format of a matrix with the screw axes as
     *the columns. thetamatd: An Nxn matrix of desired joint variables from the
     *reference trajectory dthetamatd: An Nxn matrix of desired joint velocities
     *  ddthetamatd: An Nxn matrix of desired joint accelerations
     *	gtilde: The gravity vector based on the model of the actual robot
     *(actual values given above) Mtildelist: The link frame locations based on
     *the model of the actual robot (actual values given above) Gtildelist: The
     *link spatial inertias based on the model of the actual robot (actual
     *values given above) Kp: The feedback proportional gain (identical for each
     *joint) Ki: The feedback integral gain (identical for each joint) Kd: The
     *feedback derivative gain (identical for each joint) dt: The timestep
     *between points on the reference trajectory intRes: Integration resolution
     *is the number of times integration (Euler) takes places between each time
     *step. Must be an integer value greater than or equal to 1
     *
     * Outputs: std::vector of [taumat, thetamat]
     *  taumat: An Nxn matrix of the controllers commanded joint forces/
     *torques, where each row of n forces/torques corresponds to a single time
     *instant thetamat: The N x n matrix of actual joint angles
     */
    std::vector<Eigen::MatrixXd> SimulateControl(
        const Eigen::VectorXd&, const Eigen::VectorXd&, const Eigen::VectorXd&,
        const Eigen::MatrixXd&, const std::vector<Eigen::MatrixXd>&,
        const std::vector<Eigen::MatrixXd>&, const Eigen::MatrixXd&,
        const Eigen::MatrixXd&, const Eigen::MatrixXd&, const Eigen::MatrixXd&,
        const Eigen::VectorXd&, const std::vector<Eigen::MatrixXd>&,
        const std::vector<Eigen::MatrixXd>&, double, double, double, double,
        int);
