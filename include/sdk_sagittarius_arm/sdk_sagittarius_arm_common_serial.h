...
namespace sdk_sagittarius_arm
{
    ...
    {
      public:
        CSDarmCommonSerial(const std::string& serialname, int& baudrate,
                           int& timelimit);
      ...
      protected:
        ...
        // virtual int SendDeviceReq(const char *req, std::vector<unsigned char>
        // *resp);

        virtual int SendArmAllServer(float v1, float v2, float v3, float v4,
                                     float v5, float v6);
        virtual int SendArmAllServerWithIndex(ServoStruct sv[], int num);

        virtual int SendArmAllServerTime(float v1, float v2, float v3, float v4,
                                         float v5, float v6);
        virtual int SendArmAllServerCB(float v1, float v2, float v3, float v4,
                                       float v5, float v6);
