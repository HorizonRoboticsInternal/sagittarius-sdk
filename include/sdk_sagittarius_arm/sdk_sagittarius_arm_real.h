...
namespace sdk_sagittarius_arm
{
    class SagittariusArmReal
    {
      public:
        ...
        /// 建议设置在1000左右
        /// @param acc - int 舵机的加速度，
        /// 对应舵机的加速度0～254。建议设置在10以内
        SagittariusArmReal(std::string strSerialName, int Baudrate, int vel,
                           int acc);
        ...
        /// @param dist - 夹爪的距离值
        /// -0.068~0.0(其中-0.068表示全关闭，0.0表示全开)
    ...
    class SagittariusArmKinematics
    {
      public:
        ...
        /// @brief getIKinThetaMatrix
        /// 给定末端位姿的矩阵，运算出机械臂各个关节的角度
        /// @param M - 末端在世界坐标上表达的矩阵
        /// @param theta_result「」- 运算出关节的解会存储在这个数组中
        /// @param eomg - 姿态的精度
        /// @param ev - 位置的精度
        bool getIKinThetaMatrix(const Eigen::MatrixXd& M_EE,
                                float theta_result[], double eomg = 0.001,
                                double ev = 0.001);
        ...
        /// @param eomg - 姿态的精度
        /// @param ev - 位置的精度
        bool getIKinThetaEuler(float x, float y, float z, float roll,
                               float pitch, float yaw, float theta_result[],
        ...
        /// @param eomg - 姿态的精度
        /// @param ev - 位置的精度
        bool getIKinThetaQuaternion(float x, float y, float z, float ox,
